...
@section('title', 'Search Results - MediaLAN')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold mb-2">Search Results</h1>
    <p class="text-gray-400 mb-6 sm:mb-8 text-sm sm:text-base">{{ $query ? "Results for: $query" : 'Enter a search term to find media' }}</p>

    <!-- Media Grid -->
    @if($query && count($media) > 0)
        <div class="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-4 mb-8">
            ...
            @empty
                <p class="text-gray-400 col-span-3 sm:col-span-3 md:col-span-4 lg:col-span-6">No results found for "{{ $query }}"</p>
            @endforelse
        </div>
        ...
        @endif
    @elseif($query)
        <p class="text-gray-400 text-sm sm:text-base">No results found for "{{ $query }}"</p>
    @endif
</div>
@endsection
